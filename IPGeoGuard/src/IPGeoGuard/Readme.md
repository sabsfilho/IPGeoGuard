...
WORKDIR /src
COPY ["IPGeoGuard.csproj", "IPGeoGuard/"]
RUN dotnet restore "IPGeoGuard/IPGeoGuard.csproj"

WORKDIR "/src/IPGeoGuard"
COPY . .
RUN dotnet build "IPGeoGuard.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "IPGeoGuard.csproj" \
...
Execute unit tests
```
cd "IPGeoGuard/test/IPGeoGuard.Tests"
dotnet test
```
